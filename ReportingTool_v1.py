...
try:
    ...
    query2 = """SELECT authors.name,count(authors.id) as Views
            FROM articles, authors, log
            WHERE log.path = CONCAT(\'/article/\', articles.slug)
            and articles.author = authors.id
            Group By authors.name,authors.id Order By Views Desc;"""
    query3 = """SELECT to_char(time,\'DD-MON-YYYY\'),
            COUNT(to_char(time,\'DD-MON-YYYY\')) as count
            FROM log
            GROUP BY to_char(time,\'DD-MON-YYYY\')
            ORDER BY count desc;"""
    query4 = """SELECT to_char(time,\'DD-MON-YYYY\'),
            COUNT(to_char(time,\'DD-MON-YYYY\')) as count
            FROM log
